...
namespace D1
{
                                    ...
                                    uint16_t *payload_data_buffer_3d)
    {
        ...
        message_image->height = static_cast<uint32_t>(CygLiDARD1::Sensor::Height);
        message_image->width  = static_cast<uint32_t>(CygLiDARD1::Sensor::Width);
        ...
        for (int y = 0; y < message_image->height; y++)
        {
            for (int x = 0; x < message_image->width; x++)
            {
                ...
                if (payload_data_buffer_3d[buffer_index] < CygLiDARD1::Distance::Mode3D::Maximum_Depth_3D)
                {
                                  ...
                                  PointCloudMaker &pointcloud_maker, uint16_t *distance_buffer_3d_)
    {
        ...
        pointcloud_3d->width  = CygLiDARD1::Sensor::Width;
        pointcloud_3d->height = CygLiDARD1::Sensor::Height;
        pointcloud_3d->points.resize(cyg_driver::DATA_LENGTH_3D);

        for (buffer_index = 0; buffer_index < CygLiDARD1::Sensor::Height * CygLiDARD1::Sensor::Width; buffer_index++)
        {
